...
FastAPI backend with AI-powered task/idea/reminder extraction.
"""
import os
import re
import json
...
app = FastAPI(
    title="Second Brain Inbox",
    description="AI-powered daily life organizer - dump thoughts, AI organizes",
    version="1.0.0",
)
...
DB_PATH = os.getenv("DB_PATH", "/data/second_brain.db")
...
async def classify_and_parse(text: str) -> list[Item]:
    """Use AI to classify and extract items from user input."""
...
@app.post("/api/brain-dump", response_model=BrainDumpResponse)
