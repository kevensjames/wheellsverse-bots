...
4. **Synchronous long-running requests.** The `execute` loop (≤900 s) runs inside one HTTP request; a dropped connection leaves a row in transient `executing`/`pushing` with no auto-reclaim — operator resolves manually.
