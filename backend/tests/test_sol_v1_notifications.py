...
def test_deliver_external_never_raises():
    ...
    # by default here, so no channel fires); the channel owns its own session now
...
def _db():
    ...
    models = (SolCircleTemplate, SolGroup, SolMembership, SolCycle, SolPayment, SolNotification)
