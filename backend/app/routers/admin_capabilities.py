"""Owner-only HTTP boundary for the Capability Fabric execution gateway.

Restored on the production lineage. `main.py` referenced this module behind
KAI_CAPABILITY_EXECUTION_ENABLED, but the file only ever existed on the
feat/kai-capability-fabric lineage and never landed here -- so turning the flag on was
an ImportError at startup, i.e. an outage rather than a feature flip.

CHANGE FROM THE ORIGINAL: the original mounted GET catalog routes and POST execution
routes under one `/admin/capabilities` prefix. That would convert a read-only catalog
into an execution console by flipping a flag, which is exactly what must not happen
silently. The surfaces are therefore split:

    /admin/capabilities      GET  only  -- catalog, status, invocation history
    /admin/capability-exec   POST only  -- invoke, safe-test, brain command

Both are owner-only via the SAME `require_kai_ultra` gate (no parallel auth), and both
are dormant unless the flag is on, so a disabled deployment has zero new surface.

THIN by design: it authenticates the owner, builds the authoritative owner Principal
itself (never from the request body), and delegates every decision to the ONE shared
CapabilityExecutionService. No arbitrary shell/command/path passthrough exists -- a
caller may only name a {capability_id, operation} that resolves through the server-owned
allowlist, so there is no route by which a caller reaches a provider or the harness
directly.
"""
...
#: READ-ONLY catalog surface. GET only -- asserted by test_capability_routes.py, which
#: fails if any non-GET method is ever registered here.
router = APIRouter(prefix="/admin/capabilities", tags=["capabilities"],
                   dependencies=[Depends(require_kai_ultra)])   # OWNER-ONLY, all routes

#: EXECUTION surface, deliberately a different prefix so a read-only catalog can never
#: become an action console by flag or by accident. Same owner-only gate.
exec_router = APIRouter(prefix="/admin/capability-exec", tags=["capability-exec"],
...
@exec_router.post("/{capability_id}/invoke")
...
@exec_router.post("/{capability_id}/test")
