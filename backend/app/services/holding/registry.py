...
_ENTITIES: list[HoldingEntity] = [
    ...
    HoldingEntity("narai", "NarAI", entity_type="product", operational_status="IN-REPO component",
                  ...
                  confidence=Confidence.VERIFIED),
    HoldingEntity("nexora", "Nexora", entity_type="product",
                  operational_status="in-repo product (creator/fan platform: auth/users/db + frontend + tests)",
                  stage="code present in-repo; live/revenue status unconfirmed", ownership=_HOLDCO,
                  repository="wheellsverse-bots (core/nexora_auth|users|db + frontend/nexora {creator,fan,index}.html + tests/test_nexora_users.py)",
                  risks=["2026-07 SaaS audit found a money-theft vuln (fixed in PR #31) — re-confirm current state"],
                  last_verified_at="2026-08-30",
                  data_source="re-verified 2026-08-30 in-repo (nexora auth/users/db + frontend + tests); live/revenue status still operator-confirm",
                  confidence=Confidence.UNVERIFIED),
    HoldingEntity("siteboost", "SiteBoost", entity_type="product",
                  operational_status="pipeline LIVE on app.wheellsverse.com (per SITEBOOST_LAUNCH.md 2026-06-12)",
                  stage="launch blockers resolved; real outbound pending domain warmup + Instantly key (operator action)",
                  ownership=_HOLDCO,
                  repository="wheellsverse-bots (SITEBOOST_LAUNCH.md, data/launches/siteboost)",
                  integrations=["Google Places", "Hunter", "Resend", "Instantly", "Stripe"],
                  last_verified_at="2026-08-30",
                  data_source="re-verified 2026-08-30 in-repo (SITEBOOST_LAUNCH.md 2026-06-12 point-in-time) — re-confirm live status",
                  confidence=Confidence.UNVERIFIED),
    HoldingEntity("wmos", "W-MOS", entity_type="project",
                  operational_status="in-repo portfolio supervisor (dormant, flag-gated)",
                  stage="'W-MOS Master Supervisor' autonomous cross-business sweep — kill-switch WMOS_KILL / WMOS_ORCHESTRATOR_ENABLED",
                  ownership=_HOLDCO,
                  repository="wheellsverse-bots (core/portfolio/orchestrator.py + core/opportunities.py)",
                  last_verified_at="2026-08-30",
                  data_source="re-verified 2026-08-30 in-repo (core/portfolio orchestrator+killswitch+state, opportunities scanner)",
                  confidence=Confidence.UNVERIFIED),
    HoldingEntity("suprema", "Suprema", entity_type="project",
                  operational_status="in-repo admin subsystem (proposal/scan engine)",
                  stage="/admin/supreme: status/history/latest/proposal/scan",
                  ownership=_HOLDCO,
                  repository="wheellsverse-bots (backend/app/routers/admin_supreme.py)",
                  last_verified_at="2026-08-30",
                  data_source="re-verified 2026-08-30 in-repo (admin_supreme.py router) — business scope still operator-confirm",
                  confidence=Confidence.UNVERIFIED),
