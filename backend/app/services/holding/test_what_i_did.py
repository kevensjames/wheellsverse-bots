...
def run() -> bool:
    ...
    ck("no source connected -> nothing_recorded, zero entries, every source NOT_CONNECTED, summary says 'nothing recorded'",
       none["nothing_recorded"] is True and none["entries"] == [] and set(none["sources"].values()) == {"NOT_CONNECTED"}
       and none["summary"].startswith("nothing recorded") and all(v == 0 for v in none["counts"].values()))
    ...
    ck("today(): live reconstruction over the real feeds (DB-less here -> honest, cited, or 'nothing recorded')",
       live["day"] == "2026-09-01" and live["version"] == WHAT_I_DID_VERSION
       and (live["nothing_recorded"] or all(e["source"] in feeds for e in live["entries"])))
