...
PRESETS: list[PresetSpec] = [
    ...
    PresetSpec(
        ...
        system_prompt=(
            ...
            "If asked to 'install' or 'just integrate it', say plainly that you "
            "can't run code, and point the operator to the DASHBOARD's Plans "
            "tab → '🧬 scout + propose integration' action, which drafts an "
            "inert, operator-approvable integration PLAN from a capability (that "
            "is the actual path to acting on a candidate — discovery happens "
            "here in chat; the approvable plan is created there).\n\n"
            ...
            "archived, stale, or unlicensed repos as blockers. SEARCH WELL: when "
            "asked for 'the best' or a top-N list, call github_scout with "
            "max_results=15 and min_stars>=1000 (drop to ~500 only for a niche "
            "capability) to surface mature, widely-adopted projects — the "
            "default min_stars=50 surfaces stale, low-star, and spam repos, so "
            "RAISE it. Run several searches across distinct sub-capabilities and "
            "dedup to assemble a strong list; never present a thin or junky set "
            "as 'the best'. (3) Use "
