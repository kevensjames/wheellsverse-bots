...
class ABTest:
    ...
    def record(self, variant: str, event: str, value: float = 0):
        ...
        if event == "impression":   r["impressions"] += 1
        elif event == "click":      r["clicks"] += 1
...
class ConversionAnalytics:
    ...
    def funnel(self, days: int = 30, platform: str = "", niche: str = "") -> Dict:
        ...
        if platform: events = [e for e in events if e.platform == platform]
        if niche:    events = [e for e in events if e.niche == niche]
    ...
    def attribution(self, days: int = 30) -> Dict:
        ...
        for e in events:
            ...
            if e.stage == "lead":   by_platform[p]["leads"] += 1
            if e.stage == "buyer":  by_platform[p]["buyers"] += 1
    ...
    def content_roi(self, days: int = 30) -> Dict:
        ...
        for e in events:
            ...
            if e.stage == "impression": by_content[c]["impressions"] += 1
            elif e.stage == "click":    by_content[c]["clicks"] += 1
            elif e.stage == "lead":     by_content[c]["leads"] += 1
