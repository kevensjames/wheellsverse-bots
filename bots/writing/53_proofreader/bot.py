#!/usr/bin/env python3
"""ProofreaderBot — grammar and style proofreading"""
import sys
...
class ProofreaderBot(BaseBot):
    ...
    def run(self, topic: str = None, **kwargs):
        topic = topic or self.config.get("topic", "Marketing copy review")
        self.logger.info("Running 53_proofreader: " + str(topic))

        system = (
            "You are a world-class expert in grammar and style proofreading. "
        ...
        task_title = "53_proofreader".replace("_", " ").title()
...
Be specific, professional, and immediately actionable."""
        ...
        output_text = (
            "# ProofreaderBot Output\n"
            f"**Topic:** {topic}\n"
            f"**Generated:** {datetime.now():%Y-%m-%d %H:%M:%S}\n\n"
            "---\n\n"
            f"{result}\n\n"
            "---\n"
            "*WheellsVerse 53_proofreader Bot*"
        )
        ts = datetime.now().strftime("%Y%m%d_%H%M%S")
        path = self.save_output(output_text, f"53_proofreader_{ts}.md", ext="md")
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="grammar and style proofreading")
