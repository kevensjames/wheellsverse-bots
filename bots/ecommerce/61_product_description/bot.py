#!/usr/bin/env python3
"""ProductDescriptionBot — ecommerce product descriptions"""
import sys
...
class ProductDescriptionBot(BaseBot):
    ...
    def run(self, topic: str = None, **kwargs):
        topic = topic or self.config.get("topic", "AI automation platform product")
        self.logger.info("Running 61_product_description: " + str(topic))

        system = (
            "You are a world-class expert in ecommerce product descriptions. "
        ...
        task_title = "61_product_description".replace("_", " ").title()
...
Be specific, professional, and immediately actionable."""
        ...
        output_text = (
            "# ProductDescriptionBot Output\n"
            f"**Topic:** {topic}\n"
            f"**Generated:** {datetime.now():%Y-%m-%d %H:%M:%S}\n\n"
            "---\n\n"
            f"{result}\n\n"
            "---\n"
            "*WheellsVerse 61_product_description Bot*"
        )
        ts = datetime.now().strftime("%Y%m%d_%H%M%S")
        path = self.save_output(output_text, f"61_product_description_{ts}.md", ext="md")
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="ecommerce product descriptions")
