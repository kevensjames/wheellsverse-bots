#!/usr/bin/env python3
"""CourseCreatorBot — online course curriculum design"""
import sys
...
class CourseCreatorBot(BaseBot):
    ...
    def run(self, topic: str = None, **kwargs):
        topic = topic or self.config.get("topic", "AI Automation for Entrepreneurs")
        self.logger.info("Running 63_course_creator: " + str(topic))

        system = (
            "You are a world-class expert in online course curriculum design. "
        ...
        task_title = "63_course_creator".replace("_", " ").title()
...
Be specific, professional, and immediately actionable."""
        ...
        output_text = (
            "# CourseCreatorBot Output\n"
            f"**Topic:** {topic}\n"
            f"**Generated:** {datetime.now():%Y-%m-%d %H:%M:%S}\n\n"
            "---\n\n"
            f"{result}\n\n"
            "---\n"
            "*WheellsVerse 63_course_creator Bot*"
        )
        ts = datetime.now().strftime("%Y%m%d_%H%M%S")
        path = self.save_output(output_text, f"63_course_creator_{ts}.md", ext="md")
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="online course curriculum design")
