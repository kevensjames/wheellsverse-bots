#!/usr/bin/env python3
"""PodcastGeneratorBot — podcast episode scripts"""
import sys
...
class PodcastGeneratorBot(BaseBot):
    ...
    def run(self, topic: str = None, **kwargs):
        topic = topic or self.config.get("topic", "How AI bots run your business")
        self.logger.info("Running 66_podcast_generator: " + str(topic))

        system = (
            "You are a world-class expert in podcast episode scripts. "
        ...
        task_title = "66_podcast_generator".replace("_", " ").title()
...
Be specific, professional, and immediately actionable."""
        ...
        output_text = (
            "# PodcastGeneratorBot Output\n"
            f"**Topic:** {topic}\n"
            f"**Generated:** {datetime.now():%Y-%m-%d %H:%M:%S}\n\n"
            "---\n\n"
            f"{result}\n\n"
            "---\n"
            "*WheellsVerse 66_podcast_generator Bot*"
        )
        ts = datetime.now().strftime("%Y%m%d_%H%M%S")
        path = self.save_output(output_text, f"66_podcast_generator_{ts}.md", ext="md")
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="podcast episode scripts")
