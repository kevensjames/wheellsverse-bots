#!/usr/bin/env python3
"""CaptionGeneratorBot — social media captions"""
import sys
...
class CaptionGeneratorBot(BaseBot):
    ...
    def run(self, topic: str = None, **kwargs):
        topic = topic or self.config.get("topic", "Entrepreneur working on AI")
        self.logger.info("Running 42_caption_generator: " + str(topic))

        system = (
            "You are a world-class expert in social media captions. "
        ...
        task_title = "42_caption_generator".replace("_", " ").title()
...
Be specific, professional, and immediately actionable."""
        ...
        output_text = (
            "# CaptionGeneratorBot Output\n"
            f"**Topic:** {topic}\n"
            f"**Generated:** {datetime.now():%Y-%m-%d %H:%M:%S}\n\n"
            "---\n\n"
            f"{result}\n\n"
            "---\n"
            "*WheellsVerse 42_caption_generator Bot*"
        )
        ts = datetime.now().strftime("%Y%m%d_%H%M%S")
        path = self.save_output(output_text, f"42_caption_generator_{ts}.md", ext="md")
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="social media captions")
