#!/usr/bin/env python3
"""ResearchBot — deep research and analysis reports"""
import sys
...
class ResearchBot(BaseBot):
    ...
    def run(self, topic: str = None, **kwargs):
        topic = topic or self.config.get("topic", "AI agents landscape 2025")
        self.logger.info("Running 58_research_bot: " + str(topic))

        system = (
            "You are a world-class expert in deep research and analysis reports. "
        ...
        task_title = "58_research_bot".replace("_", " ").title()
...
Be specific, professional, and immediately actionable."""
        ...
        output_text = (
            ...
            f"{result}\n\n"
            "---\n"
            "*WheellsVerse 58_research_bot Bot*"
        )
        ts = datetime.now().strftime("%Y%m%d_%H%M%S")
        path = self.save_output(output_text, f"58_research_bot_{ts}.md", ext="md")
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="deep research and analysis reports")
