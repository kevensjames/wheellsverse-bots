...
| `test_os_lab_runtimes.py` | 87 zero-framework checks (plain `python3`, pytest-discoverable). Includes a static scan asserting no `subprocess`/network/git/QEMU code path exists in any non-test module of the package (catalog, runtimes, certification), plus the M4/M5/L1/L2/L3 negative checks — round 2 adds the separator-stripped forbidden spellings and the decorated OS-lab source ids. |
| `test_os_lab_catalog.py` | 76 catalog/lifecycle checks (sibling cluster; `run()` + `test_os_lab_catalog()` like its siblings — no SystemExit at import), including the H1/M1/L4 negative checks and the `EXECUTED`-ledger gate. |
| `test_certification.py` | 63 pipeline/scanner checks over the fixtures (pytest-discoverable): typed/ordered steps, report template, bounded verdict + forbidden-literal grep, every §143 fixture escalates its step, repo-wide sweeps, no-execution/read-only source scan, secret withholding + redaction (H1), the deep freeze (L1), symlink/forbidden/oversize/**capped** walk (M2/M3), core-table single base (M6), catalog coordination. |
...
**Built (metadata + policy + tests):** catalog + lifecycle + §117 gate + verdict vocab; the three runtime records/policies; the §165 guard; the install gate + catalog binding; feature rows; `os_lab_view` assembly; the certification pipeline definition + static-only runner + §143 fixtures; **226 checks across three test files (76 catalog · 87 runtimes · 63 certification)**, including a negative test for every adversarial-review finding across both rounds (round 1 H1, M1–M6, L1–L3; round 2 H1 secret-leak, M1 verdict re-derivation, M2 capped walk, L1 deep freeze, L2 spelling normalization, L3 source containment, L4 Enum comparison).
...
**Step status vocabulary:** `PENDING / PASS / FAIL / SKIPPED / UNVERIFIED`. `new_report()` is the template — every step PENDING, verdict UNVERIFIED, `scope=NOT_RUN`, `executed` = `runtimes.EXECUTED` (all False), `authority_plane=KAI` (§165: a report is evidence, never an authority). `run_static(inventory)` runs steps 1–20 in order and marks 21–26 `SKIPPED` with note `EXECUTION_GATED`; `scope=STATIC_ONLY`. **`CertificationReport` and `StepResult` are frozen dataclasses and `steps` is a tuple** — a STATIC_ONLY report cannot be flipped to FULL/PASS in place (tested), and `catalog.record_verdict` refuses a clean verdict whose report is not `scope=FULL` with every step decided. **The freeze is deep (L1, round 2):** `report.executed` and each `StepResult.evidence` are wrapped in `types.MappingProxyType` in `__post_init__`, so `rep.executed['build'] = True` raises `TypeError` instead of flipping the honesty ledger in place. Consequence: `to_dict()` is the ONE serializer (it converts the proxies back with `dict()`); `dataclasses.asdict` cannot walk a `mappingproxy` and is not used on a report.
...
**Finding severities** reuse `security.models.Severity` values (`INFO/LOW/MEDIUM/HIGH/CRITICAL`). ≥MEDIUM fails its step. **Snippets (H1, round 2):** when the match ITSELF is the secret — anything routed to `credential_reads`, or any core row whose description names a `key|credential|secret|token|password` — the finding carries **path + line + label only, no snippet at all**. Every other step gets 20 chars *before* the match, the match, and **nothing after it**, passed through the shared `task_resolver.redact` **before** the 100-char truncation (truncating first could slice a secret out of its redactable pattern, e.g. a long `secret = "…"` whose closing quote falls past the cut). `redact` itself now knows `AKIA|ASIA|AROA` key ids. A scanned AWS key is *flagged* at core's severity (MEDIUM → `credential_reads` FAIL → `SUSPICIOUS`) but neither it, a 120-char secret value, nor any 20+ char prefix of one appears in the JSON report **or in the audited catalog history after `record_verdict`** — tested.
