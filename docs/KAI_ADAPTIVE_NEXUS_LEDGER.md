...
- **Still to build (non-blocked, next):** live governed subtitle accumulator + interruption consistency (App B sanitized SSE → bridge → subtitle buffer, only sanitized text); a dedicated `KaiSpeechInputProvider` module (mic status BROWSER_LIMITED). **EXTERNAL_BLOCKED (asset/infra only):** vendored pinned Three.js runtime + fixture GLB, production GLB, final visual/lip-sync certification (§40).
