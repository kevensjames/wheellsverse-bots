...
- **Still to build (non-blocked, next):** Avatar Lab dev route + LAB SVG face render, GLB renderer (Three.js) + capability inspector, KaiTTSProvider abstraction + voice audition (Voice Lab), microphone capture + input-side barge-in, progressive sanitized subtitles. **EXTERNAL_BLOCKED (asset only):** final GLB rendering + production visual certification.
