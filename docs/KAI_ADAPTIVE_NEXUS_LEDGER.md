...
- **`KaiSpeechInputProvider` — shipped + tested (§12/§13, mic BROWSER_LIMITED):** `kai-speech-input.js` — one provider over browser speech input with TRUTHFUL capabilities: recognition present → `status: BROWSER_LIMITED` (Chrome/webkit-only, permission-gated, often network-backed — never asserted universal); absent → `UNAVAILABLE` and `start()` refuses. Input-side barge-in (P0): while armed (KAI speaking), the FIRST speech onset fires `onSpeechStart` **exactly once per listening session** → the caller routes it to the ONE `KaiSpeechCancellationController.bargeIn()` (§14, no divergent stop path); disarmed → onset only delivers transcripts, no barge-in. All browser APIs injected (`SpeechRecognition`/`getUserMedia`) so the state machine is testable without a mic. **8 tests.** Registered in `_NEXUS_APP_MIME`. **234 tests total** (208 frontend + 26 backend).
- **Phase 12 non-asset-blocked subsystems: COMPLETE.** All engines built + tested + (where DOM-bearing) browser-verified: backend §24 boundary, embodiment state machine, viseme mapper/engine, idle-life, avatar driver (VIDEO/LAB/GLB), Avatar Lab, speech chunker/audio-queue/TTS-ranking/barge-in, GLB morph+bone registry, GLB validator, GLB renderer + driver drop-in, governed subtitle buffer, speech-input provider. **EXTERNAL_BLOCKED (asset/infra only — NOT faked, per §40):** vendored pinned Three.js runtime + fixture GLB, the production rigged GLB, live App B stream round-trip (Docker/App B down), voice audition (requires listening), final visual/lip-sync certification.
- **Adversarial review #2 (GLB renderer + subtitle core) — 9 confirmed fixes applied + regression-tested (2 refuted correctly):** a 4-lens (correctness/honesty/lifecycle/contract) + refute-biased-verify workflow (15 agents). Fixes: **(1, HIGH)** `KaiGLBRenderer` — a load resolving *after* `dispose()` resurrected the terminal DISPOSED state + leaked GPU handles → onLoad/onError now bail on DISPOSED, dispose anything the late bind created, never resurrect; **(2)** READY on an empty/degenerate morph dict or a non-face morph mesh → `_bind` now requires a non-empty dict AND picks the mesh binding the MOST critical morphs, throwing `no_morph_mesh`/`no_critical_morphs` (no fake READY, §8); **(3)** only `material.map` was disposed → track **all** PBR texture slots; **(4)** only the face mesh was tracked → track **every** mesh's geometry/materials + remove the scene root on dispose; **(5)** the `webglcontextlost` listener leaked → stored + `removeEventListener` on dispose + DISPOSED guard in `_onContextLost`; **(6, §24)** `kai-subtitles.js` default sanitizer was **fail-OPEN** (verbatim pass-through → reasoning could reach the screen) → now **fail-CLOSED**, reusing the authoritative `NexusPulse.stripReasoning` boundary, sanitize-on-read; **(7/8, §6)** the GLB driver advertised full caps / `loaded=true` from a mere `assetUrl` with no verified load → caps now gate on a real load, and a no-loader/no-renderer `load()` refuses (`no_loader`); **(9)** the renderer-path driver advertised bone caps (gaze/head_pose/breathing) the renderer doesn't drive → honest `GLB_RENDERER_CAPS` (morph-frame caps only) + `setViseme` now really drives morphs on the renderer path. Shared-texture dedupe added. **243 tests total** (217 frontend + 26 backend), 0 failures.
