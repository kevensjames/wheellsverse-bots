...
- **Still to build (non-blocked, next):** `KaiGLBRenderer` (load-state machine UNINITIALIZED→LOADING→READY/FAILED/DISPOSED + disposal + WebGL context-loss→VideoAvatarDriver fallback, injected `THREE` for testability), `GLBAvatarDriver` coefficient→morphInfluence wiring through the registry (same viseme-engine frames as LabAvatarDriver — §11 invariant), live governed subtitle accumulator + interruption consistency; a dedicated `KaiSpeechInputProvider` module. **EXTERNAL_BLOCKED (asset/infra only):** vendored Three.js runtime, production GLB, final visual/lip-sync certification (§40).
