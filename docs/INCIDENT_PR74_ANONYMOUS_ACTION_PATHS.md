...
| Sev | Item |
|---|---|
| HIGH | **Three webhook receivers are open on production right now**, measured against the service's actual configuration rather than assumed: `whatsapp` (no authentication of any kind), `payhip` (none — a forged POST wrote a fabricated sale, flipped the integration to "verified", and fired a Telegram notification), and `beehiiv` (token in a **query string**, and its docstring states it *accepts any request when the secret is unset* — `BEEHIIV_WEBHOOK_TOKEN` is unset on production, so that fail-open is live). **Correction:** I earlier listed `telegram` among these. It is **not** open — `TELEGRAM_WEBHOOK_SECRET` is set on production, so its check does fire. Its real defects are a fail-open `if expected_secret and ...` should that variable ever be unset, a non-constant-time `!=` comparison, and no idempotency. Next work item. |
