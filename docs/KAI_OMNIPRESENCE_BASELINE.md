...
| §48 security center | EXISTS | `services/security/{evidence_bus,posture,risk_score,aikido_adapter}.py` + `routers/admin_security.py` (10 read-only `/admin/cyber/*`) + `cyber-operations.html` + `kai-nexus-security.js`. Defensive-only, flag `KAI_CYBER_OPS_ENABLED`, undeployed. **This baseline records `feat/kai-cyber-operations`. On `release/kai-holding-os` none of it ships and the flag is removed.** |
