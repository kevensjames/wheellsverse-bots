...
| 8b. polish cluster: chat rate-limit, per-IP signup limiter, adapter timeouts, logout revoke, DEBUG default, log-rotation | ✅ done | `test_lane_a_polish.py` (23) — per-user chat key (hashed token, isolated) + trusted-proxy client-IP so signup limits per-attacker not one global bucket (#11); `NAI_PROVIDER_TIMEOUT_S` bounds hosted-provider calls (30s default); logout revokes the Supabase session; `DEBUG=False` default + prod boot-guard; newsyslog now rotates the `kai.*` logs the daemon actually writes. Full mid-stream failover stays out (see #7). |
| 8c. adversarial pre-deploy audit (21 agents) + 6 fixes | ✅ done | Probed what the `APP_ENV=test`/limiter-disabled suite can't. Fixed: **HIGH** rate-limit bypass (limiter keyed bearer-first while auth is cookie-first → rotating `Bearer` minted unlimited buckets; now cookie-first, re-verified 429@#31); **deploy-blocker** Stripe latch crash-looped on `rk_live_` restricted keys (the operator's runbook credential) → now accepts `sk_`/`rk_`; 429 now carries `Retry-After`; logout revokes via the refresh cookie after access-cookie expiry (was a silent half-logout); adapters `max_retries=0` (no blind non-idempotent retry/double-bill); `brain.stream` redacts raw provider errors. 1 finding (broad `_is_trusted_peer`) adversarially REFUTED — KAI binds loopback-only. |
